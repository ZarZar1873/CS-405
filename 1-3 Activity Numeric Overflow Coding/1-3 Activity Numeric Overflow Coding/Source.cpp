...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        /*
        * If the result is greater than or equal to 0, the increment is greater than the max minus the 
        * result, and the variable is not a double or long double then throw an overflow error
        * 
        * Cannot include double and long double because the result is inf
        */
        if (result >= 0 
            && (increment > (std::numeric_limits<T>::max() - result))
            && strcmp(typeid(T).name(), "double") != 0 
            && strcmp(typeid(T).name(), "long double") != 0) {
            throw std::overflow_error("OVERFLOW DETECTED: Ignoring Computation \n");
        }
        /*
        * If the result is greater than the max minus the increment and the variable is of double or
        * long double, then throw an overflow error
        */
        else if (result > (std::numeric_limits<T>::max()-increment)
            && (strcmp(typeid(T).name(), "double") == 0
                || strcmp(typeid(T).name(), "long double") == 0)) {
            throw std::overflow_error("OVERFLOW DETECTED: Ignoring Computation \n");
        }
        /*
        * Otherwise perform the calculation as normal
        */
        else {
            result += increment;
        }
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    {
        /*
        * If the decrement is greater than the results minus the min and the variable is not a signed
        * int, long, or __int64 then throw an underflow exception error
        */
        if (decrement > (result - std::numeric_limits<T>::min())
            && (strcmp(typeid(T).name(), "int") != 0
                && strcmp(typeid(T).name(), "long") != 0
                && strcmp(typeid(T).name(), "__int64") != 0)) {
            throw std::underflow_error("UNDERFLOW DETECTED: Ignoring Computation \n");
        }
        /*
        * If the result is less than 0 + the decrement and the type is a signed char, short, int, long,
        * or __int64 then throw an underflow exception error
        */
        else if (result < 0 + decrement
...
void test_overflow()
{
    ...
    /*
    * Try to perform the calculations and if sucessful then print the results
    */
    try {
    ...
    /*
    * If there is an error then print the error instead of the overflow results
    */
...
void test_underflow()
{
    ...
    */
    ...
    /*
    * If there is an error then print the error instead of the underflow results
    */
