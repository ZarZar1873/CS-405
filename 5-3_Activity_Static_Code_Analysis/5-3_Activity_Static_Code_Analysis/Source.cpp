...
class A
{
    int x{}; // add empty brackets to initialize to 0
    A(const A& other) {}
};
...
public:
    ...
    {
        // Exception in function declared to not throw exceptions. Commented out to solve error
...
{
    int buf[1001]{}; // set buf to the amount match count
    if (count == 1000)
        buf[count] = 0; // <- ERROR fixed by increasing buf to size 1001
}
...
int a;
int my_function() // function was set to bool, creating error when returning value
{
    a = 1 + 2;
    return a;
}
