...
std::string encrypt_decrypt(const std::string& source, const std::string& key)
{
  ...
  { // TODO: student need to change the next line from output[i] = source[i]
    ...
    /*
    * Format for encryption / decryption from XOR Encryption 
...
std::string read_file(const std::string& filename)
{
  ...
  file_text = ""; // Empty test string so inputdatafile and decypted file will be the same
  /*
  * Assistance from GeekForGeeks
  * https://www.geeksforgeeks.org/how-to-read-file-into-string-in-cpp/
  */
  std::ifstream file(filename); // Open the file using ifstream

  // Confirm that the file opened and print error if not
  if (!file.is_open()) {
      std::cerr << "File failed to open: " << filename << std::endl;
  }

  std::string fileLine = ""; // String for reading line of text

  while (std::getline(file, fileLine)) { // While there is still a line to read in the file
      file_text += fileLine + "\n"; // Add the next line from the file text to the file_text
  }
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
  ...
  //  Line 4+: data
    ...
    /*
    * Assistance from SheCodes for file writing
    * https://www.shecodes.io/athena/40458-how-to-write-to-an-output-file-in-c#:~:text=We%20create%20an%20object%20of,write%20data%20to%20the%20file.
    * Assitance from StackOverflow for using ctime
    * https://stackoverflow.com/questions/997946/how-to-get-current-time-and-date-in-c
    * Assistance from StackOverflow for using localtime_s for safety
    * https://stackoverflow.com/questions/35258285/how-to-use-localtime-s-with-a-pointer-in-c
    */
    std::ofstream savedDataFile(filename); // Output for file writing
    struct tm currentTime;
    time_t now = std::time(0); // Get the current time
    localtime_s(&currentTime, &now);
    

    if (savedDataFile.is_open()) { // Check to make sure file is open
        savedDataFile << student_name << std::endl;
        savedDataFile << currentTime.tm_year+1900 << "-" << currentTime.tm_mon + 1 << "-" <<
            currentTime.tm_mday << std::endl;
        savedDataFile << key << std::endl;
        savedDataFile << data << std::endl;
    }
    else { // Error catch for if the file fails to open
        std::cerr << "File failed to open: " << filename << std::endl;
    }
}
