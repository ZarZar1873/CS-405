...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  //  something more generic
    ...
    // Regular Expression that checks for non 0-9 and A-Z non case sensitive characters, the word "and" or "or",
    // or the character ' or " in single line mode
    std::regex SQLCheck("[^0-9A-Z+$]\\s*(and|or)\\s*[\\'\"]{0,1}");

    // If the regular expression above is found in the sql variable input
    if (std::regex_search(sql, SQLCheck)) {
        // Display the error message and return false
        std::cout << "Possible SQL attempt detected! Error Message: " << &error_message << std::endl;
        return false;
    }
